...
{
    public partial class EmpPayRangeList : System.Web.UI.Page
    {
        ...
        private string GetSelStr_List()
        {
            ...
            string vResultStr = "select z.Title, (select ClassTxt from DBDICB where ClassNo = z.Title and FKey = '員工資料        EMPLOYEE        TITLE') Title_C , " + Environment.NewLine +
                                "       sum(z.TitleCount) TitleCount, " + Environment.NewLine +
                                ...
                                "       select p.Title, count(p.Title) TitleCount, " + Environment.NewLine +
                                ...
                                "        where p.PayDate between '" + vPayDate_Start + "' and '" + vPayDate_End + "' and p.PayDur = '" + vPayDur + "' " + Environment.NewLine +
                                "        group by p.Title " + Environment.NewLine +
                                "        union all " + Environment.NewLine +
                                "       select p.Title, cast(0 as int) TitleCount, " + Environment.NewLine +
                                ...
                                " group by z.Title " + Environment.NewLine +
                                " order by z.Title";
            return vResultStr;
        }
        ...
        private string GetSelectStr_Excel()
        {
            ...
            string vResultStr = "select z.Title, (select ClassTxt from DBDICB where ClassNo = z.Title and FKey = '員工資料        EMPLOYEE        TITLE') Title_C , " + Environment.NewLine +
                                "       sum(z.TitleCount) TitleCount, " + Environment.NewLine +
                                "       sum(z.RCount_1) RCount_1, (sum(z.RCount_1) / cast(sum(z.TitleCount) as float)) Ratio_1, " + Environment.NewLine +
                                "       sum(z.RCount_2) RCount_2, (sum(z.RCount_2) / cast(sum(z.TitleCount) as float)) Ratio_2, " + Environment.NewLine +
                                "       sum(z.RCount_3) RCount_3, (sum(z.RCount_3) / cast(sum(z.TitleCount) as float)) Ratio_3, " + Environment.NewLine +
                                "       sum(z.RCount_4) RCount_4, (sum(z.RCount_4) / cast(sum(z.TitleCount) as float)) Ratio_4, " + Environment.NewLine +
                                "       sum(z.RCount_5) RCount_5, (sum(z.RCount_5) / cast(sum(z.TitleCount) as float)) Ratio_5, " + Environment.NewLine +
                                "       sum(z.RCount_6) RCount_6, (sum(z.RCount_6) / cast(sum(z.TitleCount) as float)) Ratio_6, " + Environment.NewLine +
                                "       sum(z.RCount_7) RCount_7, (sum(z.RCount_7) / cast(sum(z.TitleCount) as float)) Ratio_7, " + Environment.NewLine +
                                "       sum(z.RCount_8) RCount_8, (sum(z.RCount_8) / cast(sum(z.TitleCount) as float)) Ratio_8, " + Environment.NewLine +
                                "       sum(z.RCount_9) RCount_9, (sum(z.RCount_9) / cast(sum(z.TitleCount) as float)) Ratio_9 " + Environment.NewLine +
                                " from ( " + Environment.NewLine +
                                "       select p.Title, count(p.Title) TitleCount, " + Environment.NewLine +
                                "              cast(0 as int) RCount_1, cast(0 as int) RCount_2, cast(0 as int) RCount_3, cast(0 as int) RCount_4, cast(0 as int) RCount_5, " + Environment.NewLine +
                                "              cast(0 as int) RCount_6, cast(0 as int) RCount_7, cast(0 as int) RCount_8, cast(0 as int) RCount_9 " + Environment.NewLine +
        ...
        {
            sdsEmpPayRangeList.SelectCommand = GetSelStr_List();
            gridEmpPatRangeList.DataBind();
        }
        ...
        protected void bbPrint_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connPrint = new SqlConnection(vConnStr))
            {
                ...
                ReportDataSource rdsPrint = new ReportDataSource("EmpPayRangeListP", dtPrint);
                ...
                string vRecordNote = "預覽報表_員工薪資發放級距統計表" + Environment.NewLine +
        ...
        protected void bbExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connExcel = new SqlConnection(vConnStr))
            {
                ...
                if (drExcel.HasRows)
                {
                    ...
                    {
                        vHeaderText = (drExcel.GetName(i).ToUpper() == "TITLE") ? "" :
                                      (drExcel.GetName(i).ToUpper() == "TITLE_C") ? "職稱" :
                                      (drExcel.GetName(i).ToUpper() == "TITLECOUNT") ? "人數合計" :
                    ...
                    while (drExcel.Read())
                    {
                        ...
                        for (int i = 0; i < drExcel.FieldCount; i++)
                        {
                            ...
                            else if (drExcel.GetName(i).ToUpper() == "TITLECOUNT")
                            {
                                wsExcel.GetRow(vLinesNo).GetCell(i).SetCellType(CellType.Numeric);
                                wsExcel.GetRow(vLinesNo).GetCell(i).SetCellValue(double.Parse(drExcel[i].ToString()));
                                wsExcel.GetRow(vLinesNo).GetCell(i).CellStyle = csData_Int;
                    ...
                    try
                    {
                        ...
                        if (msTarget.Length > 0)
                        {
                            string vPayYMStr = ePayYear_Search.Text.Trim() + " 年 " + ePayMonth_Search.Text.Trim() + " 月";
                            string vPayDurStr = "第 " + ePayDur_Search.Text.Trim() + " 期";
                            string vRecordNote = "匯出檔案_員工薪資發放級距統計表" + Environment.NewLine +
                                                 "EmpPayList.aspx" + Environment.NewLine +
                                                 "發薪年月：" + vPayYMStr + Environment.NewLine +
                                                 "期別：" + vPayDurStr;
                            PF.InsertOperateRecord(vConnStr, vLoginID, vComputerName, vRecordNote);
